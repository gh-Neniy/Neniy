use sorted_code::sorted_match;
use std::path::{Path, PathBuf};

use super::{
    aux::{self, NodeView},
    data, id, selector, text,
};
use crate::{
    ErrorKind::Translation,
    NeniyError, Result,
    lexic::token::{BaseToken, Index, Token, TokenKind},
    synt::{self, data::DataUnit, node::Command, selector::Selector},
};

fn translate_block_data(node_view: &mut NodeView, block: Token, units: &[DataUnit]) -> Result<()> {
    node_view.push('[');
    let sign_msgs = data::translate_block_data(node_view, block, units, "=")?;
    node_view.push(']');

    if let Some((sign_msgs, key_start, key_end)) = sign_msgs {
        if sign_msgs.len() != 4 {
            return Err(NeniyError::new(
                "sign messages length is not 4".to_string(),
                Translation,
                node_view.source_code,
                key_start,
                key_end,
            ));
        }

        node_view.push_str("{front_text:{messages:");
        text::translate_lore(node_view, sign_msgs);
        node_view.push_str("}}");
    }

    Ok(())
}

fn translate_player(
    node_view: &mut NodeView,
    args: &[Token],
    selector: &Selector,
    mut entity_pos: Index,
) -> Result<usize> {
    if selector.kind == TokenKind::Id {
        node_view.push_str(node_view.extract(args[entity_pos as usize].base));
        entity_pos += 1;
    } else {
        selector::translate_selector(node_view, selector)?;
    }

    Ok(entity_pos as usize)
}

fn translate_ex_as(node_view: &mut NodeView) -> Result<()> {
    node_view.push_str("as ");

    let (args, selector) = node_view.as_selector()?;
    translate_player(node_view, args, selector, 0)?;

    Ok(())
}

fn translate_ex_at(node_view: &mut NodeView) -> Result<()> {
    node_view.push_str("at ");

    let (args, selector) = node_view.as_selector()?;
    translate_player(node_view, args, selector, 0)?;

    Ok(())
}

fn translate_ex_block(node_view: &mut NodeView) -> Result<()> {
    let (args, id_with_data) = node_view.as_id_with_data()?;
    let block = id::block_match(node_view, id_with_data.id)?;

    node_view.extend([
        node_view.extract(args[0].base), // if | unless
        " block ",
        node_view.extract(args[1].base), // x
        " ",
        node_view.extract(args[2].base), // y
        " ",
        node_view.extract(args[3].base), // z
        " ",
        block, // block
    ]);

    if !id_with_data.data.is_empty() {
        translate_block_data(node_view, id_with_data.id, &id_with_data.data)?;
    }

    Ok(())
}

fn translate_ex_ent(node_view: &mut NodeView) -> Result<()> {
    let (args, selector) = node_view.as_selector()?;

    node_view.extend([
        node_view.extract(args[0].base), // condition
        " entity ",
    ]);

    translate_player(node_view, args, selector, 1)?;

    Ok(())
}

fn translate_ex_items_block(node_view: &mut NodeView) -> Result<()> {
    let args = node_view.as_base()?;

    node_view.extend([
        node_view.extract(args[0].base), // condition
        " items block ~ ~ ~ ",
        node_view.extract(args[1].base), // container
        " ",
        id::item_match(node_view, args[2])?, // item name
    ]);

    Ok(())
}

fn translate_ex_items_ent(node_view: &mut NodeView) -> Result<()> {
    let (args, selector) = node_view.as_selector()?;

    node_view.extend([
        node_view.extract(args[0].base), // condition
        " items entity ",
    ]);

    let current_arg = translate_player(node_view, args, selector, 1)?;

    node_view.extend([
        " ",
        node_view.extract(args[current_arg].base), // container
        " ",
        id::item_match(node_view, args[current_arg + 1])?, // item name
    ]);

    Ok(())
}

fn translate_ex_score(node_view: &mut NodeView) -> Result<()> {
    let (args, selector) = node_view.as_selector()?;

    node_view.extend([
        node_view.extract(args[0].base), // condition
        " score ",
    ]);

    if args.len() == 5 {
        // for operators
        node_view.extend([
            node_view.extract(args[1].base), // player
            " ",
            node_view.extract(args[2].base), // objective
            " ",
            node_view.extract(args[3].base), // operator
            " ",
            node_view.extract(args[4].base), // second player
            " ",
            node_view.extract(args[2].base), // same objective
        ]);

        return Ok(());
    }

    let current_arg = translate_player(node_view, args, selector, 1)?;

    node_view.extend([
        " ",
        node_view.extract(args[current_arg].base), // objective
        " matches ",
        node_view.extract(args[current_arg + 1].base), // value or range
    ]);

    Ok(())
}

fn translate_ex_pos(node_view: &mut NodeView) -> Result<()> {
    let args = node_view.as_base()?;

    node_view.extend([
        "positioned ",
        node_view.extract(args[0].base), // x
        " ",
        node_view.extract(args[1].base), // y
        " ",
        node_view.extract(args[2].base), // z
    ]);

    Ok(())
}

fn translate_ex_store_score(node_view: &mut NodeView) -> Result<()> {
    node_view.push_str("store result score ");

    let (args, selector) = node_view.as_selector()?;
    let current_arg = translate_player(node_view, args, selector, 0)?;

    node_view.extend([" ", node_view.extract(args[current_arg].base)]); // objective

    Ok(())
}

fn translate_ex_store_bossbar(node_view: &mut NodeView) -> Result<()> {
    let args = node_view.as_base()?;

    node_view.extend([
        "store result bossbar ",
        node_view.extract(args[0].base), // bossbar name
        " value",
    ]);

    Ok(())
}

fn translate_ex_store_ent(node_view: &mut NodeView) -> Result<()> {
    node_view.push_str("store result entity ");

    let (args, selector) = node_view.as_selector()?;
    let current_arg = translate_player(node_view, args, selector, 0)?;

    node_view.extend([
        " ",
        node_view.extract(args[current_arg].base), // objective
        " ",
        node_view.extract(args[current_arg + 1].base), // data type
        " ",
        node_view.extract(args[current_arg + 2].base), // multiplier
    ]);

    Ok(())
}

fn translate_ex_store_storage(node_view: &mut NodeView) -> Result<()> {
    let args = node_view.as_base()?;

    node_view.extend([
        "store result storage ",
        node_view.extract(args[0].base),
        " ",
        node_view.extract(args[1].base),
        " ",
        node_view.extract(args[2].base),
        " ",
        node_view.extract(args[3].base),
    ]);

    Ok(())
}

fn translate_ex_uninited(node_view: &mut NodeView) -> Result<()> {
    node_view.push_str("unless score ");

    let (args, selector) = node_view.as_selector()?;

    let initial_len = node_view.result.len();
    let current_arg = translate_player(node_view, args, selector, 0)?;

    let player = node_view.result[initial_len..].to_string();
    let objective = node_view.extract(args[current_arg].base);

    node_view.extend([" ", objective, " = ", &player, " ", objective]);

    Ok(())
}

fn translate_ex_facing(node_view: &mut NodeView) -> Result<()> {
    node_view.push_str("facing ");

    let (args, selector) = node_view.as_selector()?;

    if selector.kind != TokenKind::Id {
        node_view.push_str("entity ");
        selector::translate_selector(node_view, selector)?;
        node_view.push_str(" feet");
    } else if args.len() == 1 {
        node_view.extend(["entity ", node_view.extract(args[0].base), " feet"]);
    } else {
        node_view.extend([
            node_view.extract(args[0].base), // x
            " ",
            node_view.extract(args[1].base), // y
            " ",
            node_view.extract(args[2].base), // z
        ])
    }

    Ok(())
}

fn translate_with_file_check(
    node_view: &mut NodeView,
    mut path: &Path,
    id: BaseToken,
    dir_name: &str,
    extension: &str,
) -> Result<()> {
    while path.file_name().unwrap() != "function"
        && let Some(parent) = path.parent()
    {
        path = parent;
    }

    if path.file_name().unwrap() != "function" {
        return Err(NeniyError {
            msg: ["directory \"", dir_name, "\" not found"].concat(),
            kind: Translation,
            start_row: 0,
            start_col: 0,
            end_row: 0,
            end_col: 0,
        });
    }

    let id_body = node_view.extract(id);
    let mut target_path = PathBuf::new();

    if dir_name == "function" {
        target_path.push(path);
    } else {
        if let Some(parent) = path.parent() {
            path = parent;
        } else {
            return Err(NeniyError {
                msg: ["namespace directory not found"].concat(),
                kind: Translation,
                start_row: 0,
                start_col: 0,
                end_row: 0,
                end_col: 0,
            });
        }

        target_path.push(path);
        target_path.push(dir_name);
    }

    target_path.push(id_body);
    target_path.set_extension(extension);

    if !target_path.is_file() {
        return Err(NeniyError::new(
            ["no such ", dir_name, ": ", id_body].concat(),
            Translation,
            node_view.source_code,
            id.start,
            id.end,
        ));
    }

    while let Some(parent) = path.parent()
        && parent.file_name().unwrap() != "data"
    {
        path = parent;
    }

    if path.parent().is_none() {
        return Err(NeniyError {
            msg: "parent directory \"data\" not found".to_string(),
            kind: Translation,
            start_row: 0,
            start_col: 0,
            end_row: 0,
            end_col: 0,
        });
    }

    node_view.extend([path.file_name().unwrap().to_str().unwrap(), ":", id_body]);
    Ok(())
}

pub fn choose_translate(node_view: &mut NodeView, path: &Path) -> Result<()> {
    sorted_match!(match node_view.command() {
        Command::Advancement => translate_advancement(node_view, path),
        Command::Attribute => translate_attribute(node_view),
        Command::BossbarAdd => translate_bossbar_add(node_view),
        Command::BossbarRemove => translate_bossbar_remove(node_view),
        Command::BossbarSet => translate_bossbar_set(node_view),
        Command::Clear => translate_clear(node_view),
        Command::Clone => translate_clone(node_view),
        Command::Damage => translate_damage(node_view),
        Command::DataGet => translate_data_get(node_view),
        Command::DataModify => translate_data_modify(node_view),
        Command::Effect => translate_effect(node_view),
        Command::Ex => translate_ex(node_view, path),
        Command::Fill => translate_fill(node_view),
        Command::Fn => translate_fn(node_view, path),
        Command::Gamerule => translate_gamerule(node_view),
        Command::Give => translate_give(node_view),
        Command::Gm => translate_gm(node_view),
        Command::Kill => translate_kill(node_view),
        Command::Loot => translate_loot(node_view, path),
        Command::Native => translate_native(node_view),
        Command::Pls => translate_pls(node_view),
        Command::Ptc => translate_ptc(node_view),
        Command::Random => translate_random(node_view),
        Command::Say => translate_say(node_view),
        Command::ScbObjAdd => translate_scb_obj_add(node_view),
        Command::ScbObjSet => translate_scb_obj_set(node_view),
        Command::ScbPlayers => translate_scb_players(node_view),
        Command::Setblock => translate_setblock(node_view),
        Command::Sm => translate_sm(node_view),
        Command::Spawnpoint => translate_spawnpoint(node_view),
        Command::Spectate => translate_spectate(node_view),
        Command::Stopsound => translate_stopsound(node_view),
        Command::Tag => translate_tag(node_view),
        Command::TeamAdd => translate_team_add(node_view),
        Command::TeamJoin => translate_team_join(node_view),
        Command::TeamModify => translate_team_modify(node_view),
        Command::Tellraw => translate_tellraw(node_view),
        Command::Time => translate_time(node_view),
        Command::Title => translate_title(node_view),
        Command::Tp => translate_tp(node_view),

        _ => Err(NeniyError {
            msg: "unknown command in choose_translate() (internal)".to_string(),
            kind: Translation,
            start_row: 0,
            start_col: 0,
            end_row: 0,
            end_col: 0,
        }),
    })
}

fn translate_advancement(node_view: &mut NodeView, path: &Path) -> Result<()> {
    node_view.push_str("advancement grant ");

    let (args, selector) = node_view.as_selector()?;
    let current_arg = translate_player(node_view, args, selector, 0)?;

    node_view.push_str(" only ");

    translate_with_file_check(
        node_view,
        path,
        args[current_arg].base,
        "advancement",
        "json",
    )?;

    Ok(())
}

fn translate_attribute(node_view: &mut NodeView) -> Result<()> {
    node_view.push_str("attribute ");

    let (args, selector) = node_view.as_selector()?;
    let current_arg = translate_player(node_view, args, selector, 0)?;

    node_view.extend([
        " ",
        id::attribute_match(node_view, args[current_arg])?,
        " base set ",
        node_view.extract(args[current_arg + 1].base), // value
    ]);

    Ok(())
}

fn translate_bossbar_add(node_view: &mut NodeView) -> Result<()> {
    let (args, text) = node_view.as_text()?;

    node_view.extend([
        "bossbar add ",
        node_view.extract(args[0].base), // bossbar name
        " ",
    ]);

    text::translate_text(node_view, text);

    Ok(())
}

fn translate_bossbar_set(node_view: &mut NodeView) -> Result<()> {
    let (args, selector) = node_view.as_selector()?;
    let submode = node_view.extract(args[1].base);

    node_view.extend([
        "bossbar set ",
        node_view.extract(args[0].base), // bossbar name
        " ",
        submode,
        " ",
    ]);

    if submode == "players" {
        translate_player(node_view, args, selector, 2)?;
    } else {
        // color or max
        node_view.push_str(node_view.extract(args[2].base));
    }

    Ok(())
}

fn translate_bossbar_remove(node_view: &mut NodeView) -> Result<()> {
    let args = node_view.as_base()?;

    node_view.extend([
        "bossbar remove ",
        node_view.extract(args[0].base), // bossbar name
    ]);

    Ok(())
}

fn translate_clear(node_view: &mut NodeView) -> Result<()> {
    node_view.push_str("clear ");

    let (args, selector, id_with_data) = node_view.as_selector_id_with_data()?;
    let current_arg = translate_player(node_view, args, selector, 0)?;

    node_view.extend([" ", id::item_match(node_view, id_with_data.id)?]);

    if !id_with_data.data.is_empty() {
        node_view.push('[');
        data::translate_item_data(node_view, &id_with_data.data, "=")?;
        node_view.push(']');
    }

    if args.len() == current_arg + 1 {
        node_view.extend([" ", node_view.extract(args[current_arg].base)]);
    }

    Ok(())
}

fn translate_clone(node_view: &mut NodeView) -> Result<()> {
    let args = node_view.as_base()?;

    node_view.extend([
        "clone ",
        node_view.extract(args[0].base), // start x
        " ",
        node_view.extract(args[1].base), // start y
        " ",
        node_view.extract(args[2].base), // start z
        " ",
        node_view.extract(args[3].base), // end x
        " ",
        node_view.extract(args[4].base), // end y
        " ",
        node_view.extract(args[5].base), // end z
        " ",
        node_view.extract(args[6].base), // from x
        " ",
        node_view.extract(args[7].base), // from y
        " ",
        node_view.extract(args[8].base), // from z
        " ",
        node_view.extract(args[9].base), // mode
    ]);

    if args.len() == 11 {
        node_view.extend([" ", node_view.extract(args[10].base)]); // [move]
    }

    Ok(())
}

fn translate_damage(node_view: &mut NodeView) -> Result<()> {
    node_view.push_str("damage ");

    let (args, selector) = node_view.as_selector()?;
    let current_arg = translate_player(node_view, args, selector, 0)?;

    node_view.extend([
        " ",
        node_view.extract(args[current_arg].base), // count
        " generic_kill",
    ]);

    Ok(())
}

fn translate_data_shared<'a>(
    node_view: &mut NodeView<'a>,
    args: &[Token],
    selector: &Selector,
) -> Result<(usize, &'a str)> {
    let current_arg = translate_player(node_view, args, selector, 0)?;
    let data_field = id::data_field_match(node_view, args[current_arg])?;

    Ok((current_arg, data_field))
}

fn translate_data_get(node_view: &mut NodeView) -> Result<()> {
    let (args, selector, indexing) = node_view.as_selector_indexing()?;

    node_view.push_str("data get entity ");

    let (_, data_field) = translate_data_shared(node_view, args, selector)?;

    node_view.extend([" ", data_field]);

    if !indexing.is_empty() {
        node_view.push_str(node_view.extract(indexing));
    }

    Ok(())
}

fn translate_data_modify(node_view: &mut NodeView) -> Result<()> {
    let (args, selector, list, indexing) = node_view.as_selector_list()?;

    node_view.push_str("data modify entity ");

    let (current_arg, data_field) = translate_data_shared(node_view, args, selector)?;

    node_view.extend([" ", data_field]);

    if !indexing.is_empty() {
        node_view.push_str(node_view.extract(indexing));
    }

    node_view.extend([
        " ",
        node_view.extract(args[current_arg + 1].base), // modify mode
        " value ",
    ]);

    if list.is_empty() {
        let raw_value = args[current_arg + 2];
        let value = node_view.extract(raw_value.base);

        if synt::aux::valid_numeric(raw_value) {
            node_view.push_str(value);
        } else {
            node_view.extend(["\"", value, "\""]);
        }
    } else {
        aux::translate_numeric_list(node_view, list, "f");
    }

    Ok(())
}

fn translate_effect(node_view: &mut NodeView) -> Result<()> {
    let (args, selector) = node_view.as_selector()?;
    let mode = node_view.extract(args[0].base);

    node_view.extend(["effect ", mode, " "]);

    let current_arg = translate_player(node_view, args, selector, 1)?;

    node_view.extend([" ", id::effect_match(node_view, args[current_arg])?]);

    if mode == "clear" {
        return Ok(());
    }

    node_view.extend([
        " ",
        node_view.extract(args[current_arg + 1].base), // duration
        " ",
        node_view.extract(args[current_arg + 2].base), // amplifier
        " true",
    ]);

    Ok(())
}

fn translate_ex_align(node_view: &mut NodeView) -> Result<()> {
    let args = node_view.as_base()?;

    node_view.extend(["align ", node_view.extract(args[0].base)]);
    Ok(())
}

fn translate_ex_anchored(node_view: &mut NodeView) -> Result<()> {
    let args = node_view.as_base()?;

    node_view.extend(["anchored ", node_view.extract(args[0].base)]);
    Ok(())
}

fn translate_ex(node_view: &mut NodeView, path: &Path) -> Result<()> {
    node_view.push_str("execute");
    let (subnodes, run_node) = node_view.as_ex()?;

    for subnode in subnodes {
        node_view.push(' ');

        let mut subnode_view = NodeView::new(node_view.result, subnode, node_view.source_code);

        sorted_match!(match subnode_view.command() {
            Command::ExAlign => translate_ex_align(&mut subnode_view)?,
            Command::ExAnchored => translate_ex_anchored(&mut subnode_view)?,
            Command::ExAs => translate_ex_as(&mut subnode_view)?,
            Command::ExAt => translate_ex_at(&mut subnode_view)?,
            Command::ExBlock => translate_ex_block(&mut subnode_view)?,
            Command::ExEnt => translate_ex_ent(&mut subnode_view)?,
            Command::ExFacing => translate_ex_facing(&mut subnode_view)?,
            Command::ExItemsBlock => translate_ex_items_block(&mut subnode_view)?,
            Command::ExItemsEnt => translate_ex_items_ent(&mut subnode_view)?,
            Command::ExPos => translate_ex_pos(&mut subnode_view)?,
            Command::ExScore => translate_ex_score(&mut subnode_view)?,
            Command::ExStoreBossbar => translate_ex_store_bossbar(&mut subnode_view)?,
            Command::ExStoreEnt => translate_ex_store_ent(&mut subnode_view)?,
            Command::ExStoreScore => translate_ex_store_score(&mut subnode_view)?,
            Command::ExStoreStorage => translate_ex_store_storage(&mut subnode_view)?,
            Command::ExUninited => translate_ex_uninited(&mut subnode_view)?,

            _ => {
                return Err(NeniyError {
                    msg: "unknown execute subcommand in translate_ex() (internal)".to_string(),
                    kind: Translation,
                    start_row: 0,
                    start_col: 0,
                    end_row: 0,
                    end_col: 0,
                });
            }
        })
    }

    node_view.push_str(" run ");

    let mut run_view = NodeView::new(node_view.result, run_node, node_view.source_code);

    choose_translate(&mut run_view, path)
}

fn translate_fill(node_view: &mut NodeView) -> Result<()> {
    let (args, id_with_data) = node_view.as_id_with_data()?;
    let block = id::block_match(node_view, id_with_data.id)?;

    node_view.extend([
        "fill ",
        node_view.extract(args[0].base), // start x
        " ",
        node_view.extract(args[1].base), // start y
        " ",
        node_view.extract(args[2].base), // start z
        " ",
        node_view.extract(args[3].base), // end x
        " ",
        node_view.extract(args[4].base), // end y
        " ",
        node_view.extract(args[5].base), // end z
        " ",
        block,
    ]);

    if !id_with_data.data.is_empty() {
        translate_block_data(node_view, id_with_data.id, &id_with_data.data)?;
    }

    node_view.extend([" ", node_view.extract(args[6].base)]); // mode

    if args.len() == 8 {
        node_view.extend([" ", id::block_match(node_view, args[7])?]); // block to be replaced
    }

    Ok(())
}

fn translate_fn(node_view: &mut NodeView, path: &Path) -> Result<()> {
    node_view.push_str("function ");

    let args = node_view.as_base()?;
    translate_with_file_check(node_view, path, args[0].base, "function", "neniy")?;

    if args.len() == 2 {
        node_view.extend([" with storage ", node_view.extract(args[1].base)]);
    }

    Ok(())
}

fn translate_gm(node_view: &mut NodeView) -> Result<()> {
    let (args, selector) = node_view.as_selector()?;

    node_view.extend(["gamemode ", id::game_mode_match(node_view, args[0])?, " "]);

    translate_player(node_view, args, selector, 1)?;

    Ok(())
}

fn translate_gamerule(node_view: &mut NodeView) -> Result<()> {
    let args = node_view.as_base()?;

    node_view.extend([
        "gamerule ",
        id::game_rule_match(node_view, args[0])?,
        " ",
        node_view.extract(args[1].base), // value
    ]);

    Ok(())
}

fn translate_give(node_view: &mut NodeView) -> Result<()> {
    node_view.push_str("give ");

    let (args, selector, id_with_data) = node_view.as_selector_id_with_data()?;
    let current_arg = translate_player(node_view, args, selector, 0)?;

    node_view.extend([" ", id::item_match(node_view, id_with_data.id)?]);

    if !id_with_data.data.is_empty() {
        node_view.push('[');
        data::translate_item_data(node_view, &id_with_data.data, "=")?;
        node_view.push(']');
    }

    if args.len() == current_arg + 1 {
        node_view.extend([" ", node_view.extract(args[current_arg].base)]); // count
    }

    Ok(())
}

fn translate_kill(node_view: &mut NodeView) -> Result<()> {
    node_view.push_str("kill ");

    let (args, selector) = node_view.as_selector()?;
    translate_player(node_view, args, selector, 0)?;

    Ok(())
}

fn translate_loot(node_view: &mut NodeView, path: &Path) -> Result<()> {
    node_view.push_str("loot spawn ~ ~ ~ loot ");

    let args = node_view.as_base()?;
    translate_with_file_check(node_view, path, args[0].base, "loot_table", "json")?;

    Ok(())
}

fn translate_native(node_view: &mut NodeView) -> Result<()> {
    let args = node_view.as_base()?;
    let native_command = BaseToken {
        start: args[0].base.start + 1,
        end: args[0].base.end - 1,
    };

    node_view.push_str(node_view.extract(native_command));

    Ok(())
}

fn translate_ptc(node_view: &mut NodeView) -> Result<()> {
    let (args, id_with_data) = node_view.as_id_with_data()?;

    node_view.extend(["particle ", id::particle_match(node_view, id_with_data.id)?]);

    if !id_with_data.data.is_empty() {
        node_view.push('{');
        data::translate_particle_data(node_view, &id_with_data.data)?;
        node_view.push('}');
    }

    node_view.extend([
        " ",
        node_view.extract(args[0].base), // x
        " ",
        node_view.extract(args[1].base), // y
        " ",
        node_view.extract(args[2].base), // z
        " ",
        node_view.extract(args[3].base), // dx
        " ",
        node_view.extract(args[4].base), // dy
        " ",
        node_view.extract(args[5].base), // dz
        " ",
        node_view.extract(args[6].base), // speed
        " ",
        node_view.extract(args[7].base), // count
        " ",
        node_view.extract(args[8].base), // mode
    ]);

    Ok(())
}

fn translate_random(node_view: &mut NodeView) -> Result<()> {
    let args = node_view.as_base()?;

    node_view.extend(["random value ", node_view.extract(args[0].base)]);
    Ok(())
}

fn translate_pls(node_view: &mut NodeView) -> Result<()> {
    let (args, selector) = node_view.as_selector()?;

    node_view.extend([
        "playsound ",
        id::sound_match(node_view, args[0])?,
        " neutral ",
    ]);

    let current_arg = translate_player(node_view, args, selector, 1)?;

    node_view.extend([
        " ",
        node_view.extract(args[current_arg].base), // x
        " ",
        node_view.extract(args[current_arg + 1].base), // y
        " ",
        node_view.extract(args[current_arg + 2].base), // z
        " ",
        node_view.extract(args[current_arg + 3].base), // volume
        " ",
        node_view.extract(args[current_arg + 4].base), // pitch
    ]);

    Ok(())
}

fn translate_say(node_view: &mut NodeView) -> Result<()> {
    let args = node_view.as_base()?;

    node_view.extend(["say ", node_view.extract(args[0].base)]);

    Ok(())
}

fn translate_scb_obj_add(node_view: &mut NodeView) -> Result<()> {
    let (args, text) = node_view.as_text()?;

    node_view.extend([
        "scoreboard objectives add ",
        node_view.extract(args[0].base), // objective
        " ",
        node_view.extract(args[1].base), // objective kind
    ]);

    if text.is_empty() {
        return Ok(());
    }

    node_view.push(' ');
    text::translate_text(node_view, text); // objective name

    Ok(())
}

fn translate_scb_obj_set(node_view: &mut NodeView) -> Result<()> {
    let args = node_view.as_base()?;

    node_view.extend([
        "scoreboard objectives setdisplay ",
        node_view.extract(args[0].base),
        " ",
        node_view.extract(args[1].base),
    ]);

    Ok(())
}

fn translate_scb_players(node_view: &mut NodeView) -> Result<()> {
    let (args, selector) = node_view.as_selector()?;
    let mut mode = node_view.extract(args[0].base);

    if mode == "opr" {
        mode = "operation";
    }

    node_view.extend(["scoreboard players ", mode, " "]);
    let current_arg = translate_player(node_view, args, selector, 1)?;

    node_view.extend([
        " ",
        node_view.extract(args[current_arg].base), // objective
    ]);

    if matches!(mode, "reset" | "get") {
        return Ok(());
    }

    node_view.extend([" ", node_view.extract(args[current_arg + 1].base)]); // operator

    if mode == "operation" {
        node_view.extend([
            " ",
            node_view.extract(args[current_arg + 2].base), // second entity
            " ",
            node_view.extract(args[current_arg].base), // same objective
        ]);
    }

    Ok(())
}

fn translate_setblock(node_view: &mut NodeView) -> Result<()> {
    let (args, id_with_data) = node_view.as_id_with_data()?;
    let block = id::block_match(node_view, id_with_data.id)?;

    node_view.extend([
        "setblock ",
        node_view.extract(args[0].base), // x
        " ",
        node_view.extract(args[1].base), // y
        " ",
        node_view.extract(args[2].base), // z
        " ",
        block, // block
    ]);

    if !id_with_data.data.is_empty() {
        translate_block_data(node_view, id_with_data.id, &id_with_data.data)?;
    }

    node_view.extend([" ", node_view.extract(args[3].base)]); // mode

    Ok(())
}

fn translate_spawnpoint(node_view: &mut NodeView) -> Result<()> {
    node_view.push_str("spawnpoint ");

    let (args, selector) = node_view.as_selector()?;
    let current_arg = translate_player(node_view, args, selector, 0)?;

    node_view.extend([
        " ",
        node_view.extract(args[current_arg].base), // x
        " ",
        node_view.extract(args[current_arg + 1].base), // y
        " ",
        node_view.extract(args[current_arg + 2].base), // z
    ]);

    Ok(())
}

fn translate_spectate(node_view: &mut NodeView) -> Result<()> {
    node_view.push_str("spectate ");

    let (args, selector) = node_view.as_selector()?;
    translate_player(node_view, args, selector, 0)?;

    Ok(())
}

fn translate_stopsound(node_view: &mut NodeView) -> Result<()> {
    node_view.push_str("stopsound ");

    let (args, selector) = node_view.as_selector()?;
    let current_arg = translate_player(node_view, args, selector, 0)?;

    node_view.extend([" * ", id::sound_match(node_view, args[current_arg])?]);
    Ok(())
}

fn translate_sm(node_view: &mut NodeView) -> Result<()> {
    let (args, id_with_data) = node_view.as_id_with_data()?;

    node_view.extend([
        "summon ",
        id::entity_match(node_view, id_with_data.id)?,
        " ",
        node_view.extract(args[0].base), // x
        " ",
        node_view.extract(args[1].base), // y
        " ",
        node_view.extract(args[2].base), // z
    ]);

    if !id_with_data.data.is_empty() {
        node_view.push_str(" {");
        data::translate_entity_data(node_view, id_with_data.id, &id_with_data.data)?;
        node_view.push('}');
    }

    Ok(())
}

fn translate_tag(node_view: &mut NodeView) -> Result<()> {
    node_view.push_str("tag ");

    let (args, selector) = node_view.as_selector()?;
    let current_arg = translate_player(node_view, args, selector, 0)?;

    node_view.extend([
        " ",
        node_view.extract(args[current_arg].base), // mode
        " ",
        node_view.extract(args[current_arg + 1].base), // tag name
    ]);

    Ok(())
}

fn translate_team_add(node_view: &mut NodeView) -> Result<()> {
    let args = node_view.as_base()?;
    node_view.extend(["team add ", node_view.extract(args[0].base)]); // team name

    Ok(())
}

fn translate_team_join(node_view: &mut NodeView) -> Result<()> {
    let (args, selector) = node_view.as_selector()?;

    node_view.extend(["team join ", node_view.extract(args[0].base), " "]); // team name
    translate_player(node_view, args, selector, 1)?;

    Ok(())
}

fn translate_team_modify(node_view: &mut NodeView) -> Result<()> {
    let args = node_view.as_base()?;
    let mut rule = node_view.extract(args[1].base);

    if rule == "friendly_fire" {
        rule = "friendlyFire";
    } else if rule == "collision" {
        rule = "collisionRule";
    }

    node_view.extend([
        "team modify ",
        node_view.extract(args[0].base), // team name
        " ",
        rule,
        " ",
        node_view.extract(args[2].base), // value
    ]);

    Ok(())
}

fn translate_tellraw(node_view: &mut NodeView) -> Result<()> {
    node_view.push_str("tellraw ");

    let (args, selector, text) = node_view.as_selector_text()?;

    translate_player(node_view, args, selector, 0)?;

    node_view.push(' ');

    text::translate_text(node_view, text);

    Ok(())
}

fn translate_time(node_view: &mut NodeView) -> Result<()> {
    let args = node_view.as_base()?;

    node_view.extend([
        "time ",
        node_view.extract(args[0].base), // mode
        " ",
        id::time_match(node_view, args[1])?, // value
    ]);

    Ok(())
}

fn translate_title(node_view: &mut NodeView) -> Result<()> {
    node_view.push_str("title ");

    let (args, selector, text) = node_view.as_selector_text()?;
    let current_arg = translate_player(node_view, args, selector, 0)?;

    node_view.extend([" ", node_view.extract(args[current_arg].base), " "]); // mode

    text::translate_text(node_view, text);

    Ok(())
}

fn translate_tp(node_view: &mut NodeView) -> Result<()> {
    node_view.push_str("tp ");

    let (args, selector1, selector2) = node_view.as_double_selector()?;

    match args.len() {
        0 => {
            // one or two selectors
            selector::translate_selector(node_view, selector1)?;

            if selector2.kind != TokenKind::Id {
                node_view.push(' ');
                selector::translate_selector(node_view, selector2)?;
            }
        }
        1 => {
            // exactly one entity name
            if selector1.kind == TokenKind::Id {
                node_view.push_str(node_view.extract(args[0].base));

                if selector2.kind != TokenKind::Id {
                    node_view.push(' ');
                    selector::translate_selector(node_view, selector2)?;
                }
            } else {
                selector::translate_selector(node_view, selector1)?;
                node_view.extend([" ", node_view.extract(args[0].base)]);
            }
        }
        2 => {
            // both arguments are entity names
            node_view.extend([
                node_view.extract(args[0].base),
                " ",
                node_view.extract(args[1].base),
            ]);
        }
        3 => {
            // only coordinates
            if selector1.kind != TokenKind::Id {
                selector::translate_selector(node_view, selector1)?;
                node_view.push(' ');
            }

            node_view.extend([
                node_view.extract(args[0].base),
                " ",
                node_view.extract(args[1].base),
                " ",
                node_view.extract(args[2].base),
            ]);
        }
        4 => {
            // entity name and coordinates
            node_view.extend([
                node_view.extract(args[0].base),
                " ",
                node_view.extract(args[1].base),
                " ",
                node_view.extract(args[2].base),
                " ",
                node_view.extract(args[3].base),
            ])
        }
        5 => {
            // only coordinates
            if selector1.kind != TokenKind::Id {
                selector::translate_selector(node_view, selector1)?;
                node_view.push(' ');
            }

            node_view.extend([
                node_view.extract(args[0].base),
                " ",
                node_view.extract(args[1].base),
                " ",
                node_view.extract(args[2].base),
                " ",
                node_view.extract(args[3].base),
                " ",
                node_view.extract(args[4].base),
            ]);
        }
        6 => {
            // one entity name and 5 coordinates
            node_view.extend([
                node_view.extract(args[0].base),
                " ",
                node_view.extract(args[1].base),
                " ",
                node_view.extract(args[2].base),
                " ",
                node_view.extract(args[3].base),
                " ",
                node_view.extract(args[4].base),
                " ",
                node_view.extract(args[5].base),
            ]);
        }

        _ => {
            return Err(NeniyError {
                msg: "unknown execute subcommand in translate_ex() (internal)".to_string(),
                kind: Translation,
                start_row: 0,
                start_col: 0,
                end_row: 0,
                end_col: 0,
            });
        }
    }

    Ok(())
}
